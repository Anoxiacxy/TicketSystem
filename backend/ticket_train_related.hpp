...
namespace sjtu{
	...
	public:
		...
		priority_queue<ticket_train, cmp_time> query_ticket_time(const string<20> &station1, const string<20> &station2, const string<5> &date){
			...
			while(iter1.get_key().get_station() == station1 && iter2.get_key().get_station() == station2){
				if(iter1.get_key().get_trainID() < iter2.get_key().get_trainID()){
					++iter1;
				}
				else if(iter1.get_key().get_trainID() > iter2.get_key().get_trainID()){
					++iter2;
				}
				else{
					if(iter1->station_num >= iter2->station_num){
						...
						string<5> tmpdate = minus_date(date, iter1->date_fix);
						if(iter1->saleDate_from <= tmpdate && iter2->saleDate_to >= tmpdate){
							result.push(ticket_train(iter1.get_key().get_trainID(), station1, station2, minus_date(date, iter1->date_fix), date, add_date(date, iter2->date_fix - iter1->date_fix + iter2->date_gap), iter1->leaving_time, iter2->arriving_time, iter2->price - iter1->price, iter1->station_num, iter2->station_num));
						}
						++iter1;
						++iter2;
					}
		...
		pair<ticket_train, ticket_train> query_transfer_time(const string<20> &station1, const string<20> &station2, const string<5> &date, train_related &T){
			...
			pair<ticket_train, ticket_train> ans; 
			char a[2];
			a[0] = 0x80;
			string<20> tmpID = string<20>(a);
			auto iter1 = ticket_train_tree.lower_bound(station_train(station1, tmpID));
			...
			while(iter1.get_key().get_station() == station1){
				...
				if(!tmp.isrelease()){
			        ...
			        for(int i = 0; i < tmp.stationNum; ++i){
			        	if(flag){
			        		set1.insert(pair<key, string<20> >(key{tmp.stations[i], arriving_time, leaving_time1, date_gap, date_fix1 - date_fix, date_fix, price - price1, tmp.saleDate_from, tmp.saleDate_to, num, i}, TrainID));
						}
			...
			while(iter2.get_key().get_station() == station1){
				string<20> TrainID = iter1.get_key().get_trainID();
				train tmp = T.at(TrainID);
				if(!tmp.isrelease()){
					++iter1;
			        ...
			        for(int i = 0; i < tmp.stationNum; ++i){
						...
						set2.insert(pair<key, string<20> >(key{tmp.stations[i], arriving_time1, leaving_time, date_gap1, date_fix1 - date_fix, date_fix, price - price1, tmp.saleDate_from, tmp.saleDate_to, i, num}, TrainID));
			            price += tmp.prices[i + 1];
			            pair<int, string<5> > tmpTime = add_time(leaving_time, tmp.travelTimes[i + 1]);
				        date_fix += tmpTime.first;
				        pair<int, string<5> > tmpTime2 = add_time(leaving_time, tmp.travelTimes[i + 1] + tmp.stopoverTimes[i + 1]);
			...
			for(; it1 != set1.end(); ++it1){
				for(; it2 != set2.end(); ++it2){
					...
					if(add_date(date, it1->first.date_fix1) < add_date(it2->first.saledate_to, it2->first.date_fix2) || (add_date(date, it1->first.date_fix1) == add_date(it2->first.saledate_to, it2->first.date_fix2) && it1->first.timearrive < it2->first.timeleave)){
						string<5> tmpdate;
						if(add_date(date, it1->first.date_fix1 + it1->first.date_gap) < add_date(it2->first.saledate_from, it2->first.date_fix2)) tmpdate = add_date(it2->first.saledate_from, it2->first.date_fix2);
						else{
							tmpdate = add_date(date, it1->first.date_fix1);
							if(it1->first.timearrive > it2->first.timeleave) tmpdate = add_date(tmpdate, 1);
						}
						if(minus_time(it1->first.timeleave, it2->first.timearrive) + 1440 * minus_date(date, add_date(tmpdate, it2->first.date_fix1 + it2->first.date_gap)));
					}
				}
			}
		}
		pair<ticket_train, ticket_train> query_transfer_price(const string<20> &station1, const string<20> &station2, const string<5> &date, train_related &T){
			...
			while(iter2.get_key().get_station() == station1){
				...
				if(!tmp.isrelease()){
			        ...
			        for(int i = 0; i < tmp.stationNum; ++i){
						...
						set2.insert(pair<key, string<20> >(key{tmp.stations[i], arriving_time1, leaving_time, date_gap1, date_fix1 - date_fix, date_fix, price1 - price, tmp.saleDate_from, tmp.saleDate_to, i, num}, TrainID));
			            price += tmp.prices[i + 1];
			            pair<int, string<5> > tmpTime = add_time(leaving_time, tmp.travelTimes[i + 1]);
				        date_fix += tmpTime.first;
				        pair<int, string<5> > tmpTime2 = add_time(leaving_time, tmp.travelTimes[i + 1] + tmp.stopoverTimes[i + 1]);
			...
			for(; it1 != set1.end(); ++it1){
				for(; it2 != set2.end(); ++it2){
					...
					if(add_date(date, it1->first.date_fix1) < add_date(it2->first.saledate_to, it2->first.date_fix2) || (add_date(date, it1->first.date_fix1) == add_date(it2->first.saledate_to, it2->first.date_fix2) && it1->first.timearrive <= it2->first.timeleave)){
						if(it1->first.price + it2->first.price < min){
							min = it1->first.price + it2->first.price;
							ans1 = ticket_train(it1->second, station1, it1->first.station, minus_date(date, it1->first.date_fix2 - it1->first.date_fix1), date, add_date(date, it1->first.date_fix1 + it1->first.date_gap), it1->first.timeleave, it1->first.timearrive, it1->first.price, it1->first.num1, it1->first.num2);
							string<5> tmpdate;
							if(add_date(date, it1->first.date_fix1 + it1->first.date_gap) < add_date(it2->first.saledate_from, it2->first.date_fix2)) tmpdate = add_date(it2->first.saledate_from, it2->first.date_fix2);
							else{
								tmpdate = add_date(date, it1->first.date_fix1);
							...
							ans2 = ticket_train(it2->second, it2->first.station, station2, minus_date(tmpdate, it2->first.date_fix2), tmpdate, add_date(tmpdate, it2->first.date_fix1 + it2->first.date_gap), it2->first.timeleave, it2->first.timearrive, it2->first.price, it2->first.num1, it2->first.num2);
						}
					}
				}
			}
